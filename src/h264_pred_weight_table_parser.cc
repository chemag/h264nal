...
#ifdef FPRINT_ERRORS
    fprintf(stderr,
            "invalid luma_log2_weight_denom: %" PRIu32
            " not in range "
            "[%" PRIu32 ", %" PRIu32 "]\n",
            pred_weight_table->luma_log2_weight_denom, kLumaLog2WeightDenomMin,
            kLumaLog2WeightDenomMax);
#endif  // FPRINT_ERRORS
    return nullptr;
  }
...
void H264PredWeightTableParser::PredWeightTableState::fdump(
  ...
  for (const int32_t& v : luma_weight_l0) {
    fprintf(outfp, " %i", v);
  }
  fprintf(outfp, " }");
  ...
  if (chroma_array_type != 0) {
    ...
    for (const std::vector<int32_t>& vv : chroma_weight_l0) {
      fprintf(outfp, " {");
    ...
    for (const std::vector<int32_t>& vv : chroma_offset_l0) {
      fprintf(outfp, " {");
      for (const int32_t& v : vv) {
        fprintf(outfp, " %i", v);
      }
  ...
  if ((slice_type == SliceType::B) ||
    ...
    for (const int32_t& v : luma_weight_l1) {
      fprintf(outfp, " %i", v);
    }
    fprintf(outfp, " }");
    ...
    for (const int32_t& v : luma_offset_l1) {
      fprintf(outfp, " %i", v);
    }
    fprintf(outfp, " }");
    ...
    if (chroma_array_type != 0) {
      ...
      for (const std::vector<int32_t>& vv : chroma_weight_l1) {
        fprintf(outfp, " {");
        for (const int32_t& v : vv) {
          fprintf(outfp, " %i", v);
        }
      ...
      for (const std::vector<int32_t>& vv : chroma_offset_l1) {
        fprintf(outfp, " {");
        for (const int32_t& v : vv) {
          fprintf(outfp, " %i", v);
        }
