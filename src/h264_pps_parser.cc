...
std::shared_ptr<H264PpsParser::PpsState> H264PpsParser::ParsePps(
...
#ifdef FPRINT_ERRORS
      fprintf(stderr,
              "invalid slice_group_map_type: %" PRIu32
              " not in range "
              "[%" PRIu32 ", %" PRIu32 "]\n",
              pps->slice_group_map_type, kSliceGroupMapTypeMin,
              kSliceGroupMapTypeMax);
#endif  // FPRINT_ERRORS
      return nullptr;
    }
  ...
  if (pps->num_ref_idx_l0_default_active_minus1 <
          kNumRefIdxL0DefaultActiveMinus1Min ||
      pps->num_ref_idx_l0_default_active_minus1 >
          kNumRefIdxL0DefaultActiveMinus1Max) {
#ifdef FPRINT_ERRORS
    fprintf(stderr,
            "invalid num_ref_idx_l0_default_active_minus1: %" PRIu32
            " not in range "
            "[%" PRIu32 ", %" PRIu32 "]\n",
            pps->num_ref_idx_l0_default_active_minus1,
            kNumRefIdxL0DefaultActiveMinus1Min,
