...
// (`pred_weight_table()`, as defined in Section 7.3.3.2 of the 2004
...
class H264PredWeightTableParser {
 public:
  ...
  static std::unique_ptr<PredWeightTableState> ParsePredWeightTable(
      const uint8_t* data, size_t length, uint32_t slice_type,
      uint32_t num_ref_idx_l0_active_minus1,
      uint32_t num_ref_idx_l1_active_minus1) noexcept;
  static std::unique_ptr<PredWeightTableState> ParsePredWeightTable(
      rtc::BitBuffer* bit_buffer, uint32_t slice_type,
      uint32_t num_ref_idx_l0_active_minus1,
      uint32_t num_ref_idx_l1_active_minus1) noexcept;
};
