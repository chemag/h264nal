...
H264SliceHeaderParser::ParseSliceHeader(
  ...
  if (slice_header->first_mb_in_slice > (kMaxMbPicSize - 1)) {
#ifdef FPRINT_ERRORS
    fprintf(stderr,
  ...
  if (slice_header->pic_parameter_set_id < kPicParameterSetIdMin ||
    ...
    fprintf(stderr,
            ...
            " not in range "
            "[%" PRIu32 ", %" PRIu32 "]\n",
            slice_header->pic_parameter_set_id, kPicParameterSetIdMin,
  ...
  if (slice_header->separate_colour_plane_flag) {
    // colour_plane_id  u(2)
    if (!bit_buffer->ReadBits(2, slice_header->colour_plane_id)) {
      return nullptr;
    }
  }
  ...
  if (!bit_buffer->ReadBits(frame_num_len, slice_header->frame_num)) {
    return nullptr;
  }
  ...
  if (!slice_header->frame_mbs_only_flag) {
    // field_pic_flag  u(1)
    if (!bit_buffer->ReadBits(1, slice_header->field_pic_flag)) {
      return nullptr;
    }
    if (slice_header->field_pic_flag) {
      // bottom_field_flag  u(1)
      if (!bit_buffer->ReadBits(1, slice_header->bottom_field_flag)) {
        return nullptr;
      }
    }
  }
  ...
  if (slice_header->redundant_pic_cnt_present_flag) {
    ...
    if (!bit_buffer->ReadExponentialGolomb(slice_header->redundant_pic_cnt)) {
      return nullptr;
    }
    if (slice_header->redundant_pic_cnt < kRedundantPicCntMin ||
      ...
      fprintf(stderr,
              ...
              " not in range "
              "[%" PRIu32 ", %" PRIu32 "]\n",
              slice_header->redundant_pic_cnt, kRedundantPicCntMin,
              kRedundantPicCntMax);
#endif  // FPRINT_ERRORS
      return nullptr;
    }
  ...
  if ((slice_header->slice_type == SliceType::B) ||
    ...
    if (!bit_buffer->ReadBits(1, slice_header->direct_spatial_mv_pred_flag)) {
      return nullptr;
    }
  }
  ...
  if ((slice_header->slice_type == SliceType::P) ||
    ...
    if (slice_header->num_ref_idx_active_override_flag) {
      ...
      if ((slice_header->slice_type == SliceType::B) ||
        ...
        if (slice_header->num_ref_idx_l1_active_minus1 <
  ...
  if ((slice_header->slice_type == SliceType::SP) ||
    ...
    if ((slice_header->slice_type == SliceType::SP) ||
      ...
      if (!bit_buffer->ReadBits(1, slice_header->sp_for_switch_flag)) {
        return nullptr;
      }
    }
